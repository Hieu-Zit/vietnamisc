<?php
// [bt_back_to_top back_to_top ="true" back_to_top_text="value" back_to_top_style="value"]

if ( ! function_exists( 'bt_back_to_top_func' ) ) {
	function bt_back_to_top_func( $atts ) {
		...
		wp_enqueue_style( 
				'bt-back-to-top-style', 
				plugins_url( 'css/back_to_top.css', __FILE__ ),
				array(), false, 'screen' 
		);	 
		wp_enqueue_script(
		...
		if ( $back_to_top_text != '' ) {
			$class = array( 'bt_back_to_top_button_no_icon' );
		}else{
		...
		$output = '';	
		if ( $back_to_top ){
			$back_to_top_class = $back_to_top_style == 'alternate' ? ' bt_bb_back_to_top_alternate' : '';
			
			$output .= '<span class="bt_bb_back_to_top' . $back_to_top_class . '">';
